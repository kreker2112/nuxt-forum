  ...
  <div class="flex dark:bg-slate-800">
    <aside class="h-screen sticky top-0">
      ...
      <div class="mt-20 hidden lg:block">
        ...
        >
          <div
            @click="setSelectedLesson(category?.name)"
          ...
          >
            ...
            <div class="flex items-start justify-between w-full">
              <div class="w-full">
                ...
                <p
                  tabindex="0"
                  class="focus:outline-none text-sm leading-normal text-gray-500 dark:text-gray-200"
                >
          ...
          <div class="">
            <p
              tabindex="0"
              class="focus:outline-none text-sm leading-5 dark:text-white"
            >
              {{ category.message }}
            </p>
          </div>
...
const lessonName = ref(null as string | null);
const firstLoad = ref(true);

const seriesName = getParam("name") as string;
const { data: categories, pending: categoriesPending } = await useFetch<
  ICategory[]
>(`/api/series/${seriesName}`, { key: seriesName });

if (!categoriesPending.value) {
  console.log("dude", categories);
  lessonName.value = categories.value ? categories.value[0]?.name : null;
}
...
async function setSelectedLesson(name: String | null) {
  if (name !== null) {
    lessonName.value = name as string;

    const {
      data: selected,
      pending: selectFectchPending,
      refresh: refreshSelected,
    } = await useFetch<ILesson>(`/api/lesson/${name}`, {
      key: route.fullPath + name,
      server: false,
    });
