<template>
  <div v-if="data" class="md:flex dark:bg-black">
    <aside
      class="hidden md:block pl-10 pr-10 text-left h-screen w-1/4 dark:bg-slate-900"
    >
      <div class="text-gray-800 dark:text-gray-100 text-xl">
        <div class="p-2.5 mt-1 flex items-center">
          <NuxtLink to="/">
            ...
            />
            <h1 class="font-bold text-gray-800 dark:text-gray-200 text-[15px]">
              Full Stack Jack
            </h1>
          </NuxtLink>
        </div>
      ...
      <div>
        <img
          class="h-16"
          :src="data?.topic.image ?? undefined"
          v-if="data?.topic.image !== '/img/nuxt3.svg'"
          alt="nuxt 3 logo"
        />
        <svg
          v-if="data?.topic.image == '/img/nuxt3.svg'"
      ...
      <ul class="text-left mx-auto text-gray-800 dark:text-gray-200 font-bold">
        <li
          ...
          :class="{ 'bg-green-600': isActive(video) }"
        ...
        <div class="w-full min-w-200">
          ...
          >
            <div class="video__youtube" :id="activeVideo.host_id">
              <img
                src="https://i.ytimg.com/vi/${activeVideo.value.host_id}/maxresdefault.jpg"
    ...
    <div class="md:hidden">
      <ul
        class="mx-auto text-left mx-auto text-gray-800 dark:text-gray-200 font-bold"
      >
        <li
          class="cursor-pointer p-2 hover:bg-green-600 rounded-md mt-1"
          @click="setActive(video)"
          v-for="video in data?.videos"
        >
          {{ video.title }}
...
function isActive(video: Video) {
  return activeVideo.value && video.host_id === activeVideo.value.host_id;
}
...
function createIframe(vidId: string) {
  if (videoWrapper.value) {
    ...
    console.error("Video wrapper element is not available.");
  }
}
